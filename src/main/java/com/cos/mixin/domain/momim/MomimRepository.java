...
public interface MomimRepository extends JpaRepository<Momim, Long>{
	...
	Optional<Momim> findByMomimCode(String momimCode);
	...
	@Query(value = "UPDATE momim SET categoryId = (SELECT id FROM category WHERE categoryName = :categoryName), momimLeaderId = :momimLeaderId WHERE momimName = :momimName", nativeQuery = true)
